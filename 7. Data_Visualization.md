...
f. Plotly
i.	Interactivity out of the box
	•	Unlike Matplotlib or Seaborn (mostly static), Plotly charts are interactive by default.
	•	Hover tooltips, zoom, pan, select ranges — all without extra coding.
	•	Great for business users who want to explore data, not just look at a static image.
ii.	Streamlit & Dash friendly
	•	Plotly integrates seamlessly with Streamlit, Dash, and web apps.
	•	st.plotly_chart() drops in an interactive chart instantly.
	•	Perfect for the “Visual” part of your pipeline.
iii.	Rich chart types
	•	Beyond bar/line/pie: Sankey, Choropleth maps, Treemaps, 3D scatter, Candlestick, etc.
	•	Saves you time if you need less common viz types.
iv.	Publication-ready but interactive
	•	Good balance between aesthetic quality and practical functionality.
	•	Easy to export to HTML for sharing interactive reports.
v.	No JavaScript required
	•	Normally, interactive charts need JS/D3. With Plotly, you write pure Python (or R, Julia).
